...
import {
  addLookupValueSchema,
  addLookupValuesBulkSchema,
  createLookupSchema,
  lookupIdParamSchema,
  lookupValueIdParamSchema,
  searchLookupValuesSchema,
} from "../../validations/v1";
...
export class LookupController {
  ...
  static async createLookup(req: Request, res: Response): Promise<void> {
    try {
      const {
        ...
        allowLeftRightDups,
        strictChecking,
      } = validateBody(req, createLookupSchema);
  ...
  static async addLookupValue(req: Request, res: Response): Promise<void> {
    try {
      const { left, right, leftMetadata, rightMetadata } = validateBody(
        req,
        addLookupValueSchema
      );
      const { lookup } = validateParams(req, lookupIdParamSchema);
      ...
      const value = await prisma.lookupValue.create({
        data: {
          ...
          rightMetadata: rightMetadata
            ? (rightMetadata as any)
            : Prisma.JsonNull,
        },
      });

      logger.info("Lookup value added", {
        lookupId: lookupData.id,
        valueId: value.id,
        left,
        right,
          ...
          try {
            ...
            const newValue = await prisma.lookupValue.create({
              data: {
                ...
                leftMetadata: valueData.leftMetadata
                  ? (valueData.leftMetadata as any)
                  : Prisma.JsonNull,
                rightMetadata: valueData.rightMetadata
                  ? (valueData.rightMetadata as any)
                  : Prisma.JsonNull,
              },
            });
            results.created++;
  ...
  static async searchLookupValues(req: Request, res: Response): Promise<void> {
    try {
      const { left, right, search } = validateQuery(
        req,
        searchLookupValuesSchema
      );
      const { lookup } = validateParams(req, lookupIdParamSchema);
  ...
  static async removeLookupValue(req: Request, res: Response): Promise<void> {
    try {
      ...
      logger.info("Lookup value removed", {
        lookupId: lookupData.id,
        valueId: value.id,
      });
      res.json({
  ...
  /**
   * Get all values for a lookup
   * GET /api/v1/lookups/:lookup/values
   */
  static async getLookupValues(req: Request, res: Response): Promise<void> {
    try {
      const { lookup } = validateParams(req, lookupIdParamSchema);
      ...
      logger.error("Error fetching lookup values", { error });
      res.status(500).json({
        success: false,
        error: "Internal server error",
        code: "INTERNAL_ERROR",
  ...
  /**
   * Update a specific lookup value
   * PUT /api/v1/lookups/:lookup/values/:valueId
   */
  static async updateLookupValue(req: Request, res: Response): Promise<void> {
    try {
      const { lookup, valueId } = validateParams(req, lookupValueIdParamSchema);
      const { left, right, leftMetadata, rightMetadata } = validateBody(
        req,
      ...
      const updatedValue = await prisma.lookupValue.update({
        where: {
          id: valueId,
          lookupId: lookupData.id,
        },
      ...
      logger.error("Error updating lookup value", { error });
      res.status(500).json({
        success: false,
        error: "Internal server error",
        code: "INTERNAL_ERROR",
  ...
  /**
   * Clear all values from a lookup
   * DELETE /api/v1/lookups/:lookup/values
   */
  static async clearLookupValues(req: Request, res: Response): Promise<void> {
    try {
      const { lookup } = validateParams(req, lookupIdParamSchema);
      ...
      logger.error("Error clearing lookup values", { error });
      res.status(500).json({
        success: false,
        error: "Internal server error",
        code: "INTERNAL_ERROR",
  ...
  /**
   * Delete multiple lookup values
   * POST /api/v1/lookups/:lookup/values/delete-list
   */
  static async deleteMultipleLookupValues(
    req: Request,
    res: Response
  ): Promise<void> {
    try {
      ...
      const result = await prisma.lookupValue.deleteMany({
        where: {
          id: { in: valueIds },
          lookupId: lookupData.id,
        },
      ...
      logger.error("Error deleting multiple lookup values", { error });
      res.status(500).json({
        success: false,
        error: "Internal server error",
        code: "INTERNAL_ERROR",
   ...
   * PUT /api/v1/lookups/:lookup
   */
  static async updateLookup(req: Request, res: Response): Promise<void> {
    try {
      const { lookup } = validateParams(req, lookupIdParamSchema);
      const {
        name,
      ...
      const updatedLookup = await prisma.lookup.update({
        where: { id: lookupData.id },
        data: {
          ...(name && { name }),
          ...(description !== undefined && { description }),
          ...(leftSystem && { leftSystem }),
          ...(rightSystem && { rightSystem }),
          ...(allowLeftDups !== undefined && { allowLeftDups }),
          ...(allowRightDups !== undefined && { allowRightDups }),
          ...(allowLeftRightDups !== undefined && { allowLeftRightDups }),
      ...
      if (error.code === "P2002") {
        res.status(409).json({
          success: false,
          error: "Lookup name already exists",
  ...
  /**
   * Delete a lookup
   * DELETE /api/v1/lookups/:lookup
   */
  static async deleteLookup(req: Request, res: Response): Promise<void> {
    try {
      const { lookup } = validateParams(req, lookupIdParamSchema);
